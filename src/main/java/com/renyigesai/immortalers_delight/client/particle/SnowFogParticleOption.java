...
public class SnowFogParticleOption implements ParticleOptions {
    ...
    public static final Codec<SnowFogParticleOption> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
    ...
    public static final Deserializer<SnowFogParticleOption> DESERIALIZER = new Deserializer<>() {
        // 从指令解析参数（如/particle yourmod:extra_raidus ~ ~ ~ 0 0 0 0 1 10）
        @Override
        public SnowFogParticleOption fromCommand(ParticleType<SnowFogParticleOption> type, StringReader reader) throws CommandSyntaxException {
            reader.expect(' '); // 跳过空格
            int extra_size = reader.readInt(); // 读取指令中的int参数
            return new SnowFogParticleOption(extra_size);
        }

        // 从网络缓冲区读取参数（服务端→客户端）
        @Override
        public SnowFogParticleOption fromNetwork(ParticleType<SnowFogParticleOption> type, FriendlyByteBuf buffer) {
            return new SnowFogParticleOption(buffer.readVarInt()); // 读取VarInt类型的int
        }
    };
    ...
    public @NotNull ParticleType<SnowFogParticleOption> getType() {
        return ImmortalersDelightParticleTypes.SNOW_FOG.get(); // 替换为你的粒子类型注册引用
    }
